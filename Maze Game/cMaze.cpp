...
void cMaze::PlaceObject(GameObject* obj)
{
	p_Cells[obj->GetPosition().x][obj->GetPosition().y].display = obj->GetSymbol();
	p_Cells[obj->GetPosition().x][obj->GetPosition().y].obj = obj;
	...
	COORD coord = { obj->GetPosition().y ,obj->GetPosition().x };

	SetConsoleCursorPosition(hOut, coord);
	
	SetConsoleTextAttribute(hOut, obj->GetColor());

	char objSymbol = obj->GetSymbol();

	WriteConsoleA(hOut, &objSymbol, 1, 0, nullptr);
...
void cMaze::RemoveObject(cVector2 pos)
{
	p_Cells[pos.x][pos.y].display = m_floorChar;
	p_Cells[pos.x][pos.y].obj = nullptr;
...
cVector2 cMaze::GetStartPos()
{
	//We inverse the position because the command line takes the coordinates in reverse
	cVector2 inversePos(m_start.y, m_start.x);
	return inversePos;
}

cVector2 cMaze::GetExitPos()
{
	cVector2 inversePos(m_exit.y, m_exit.x);
	return inversePos;
}
...
Cell& cMaze::CheckUp(cVector2 pos)
{
	return p_Cells[pos.x - 1][pos.y];
}


Cell& cMaze::CheckDown(cVector2 pos)
{
	return p_Cells[pos.x + 1][pos.y];
}

Cell& cMaze::CheckRight(cVector2 pos)
{
	return p_Cells[pos.x][pos.y + 1];
}

Cell& cMaze::CheckLeft(cVector2 pos)
{
	return p_Cells[pos.x][pos.y - 1];
}


cVector2 cMaze::GetRandomPos()
{
	cVector2 newPos;
	int x = m_mtRand() % (m_dimensions.x - 2) + 1;
	int y = m_mtRand() % (m_dimensions.y - 2) + 1;

	//Finds cells that are set as a floor character, in this case it is ' '.
	while (p_Cells[y][x].display != m_floorChar)
	{
		x = m_mtRand() % (m_dimensions.x - 2);
		y = m_mtRand() % (m_dimensions.y - 2);
...
cVector2 cMaze::GetRandomExitPos()
{
	...
	cVector2 inversePos(randomPos.y, randomPos.x);
	return inversePos;
}
...
//-----------------------------------------------------------------------------
void        cMaze::print(int startRow, int startCol)
{
	HANDLE hOut = GetStdHandle(STD_OUTPUT_HANDLE);
